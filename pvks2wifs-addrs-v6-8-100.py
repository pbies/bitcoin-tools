...
def go(k):
	k=k.decode().strip()
	i=int(k, 16)
	for j in range(i-8, i+101):
		l=hex(j)[2:].zfill(64)
		try:
			hdwallet1 = HDWallet(cryptocurrency=BTC, hd=BIP32HD).from_private_key(private_key=l)
			hdwallet2 = HDWallet(cryptocurrency=BTC, hd=BIP84HD).from_private_key(private_key=l)
		except:
			return
