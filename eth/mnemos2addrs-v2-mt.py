...
def go(x):
	x=x.rstrip(b'\n').decode()
	address = get_ethereum_address(x, 0)
	if address is not None:
		outfile.write(f'{address} {x}\n')
	address = get_ethereum_address(x, 1)
	if address is not None:
		outfile.write(f'{address} {x}\n')
	outfile.flush()

def get_ethereum_address(mnemonic, index=0):
	try:
		seed = Bip39SeedGenerator(mnemonic).Generate()
		bip44_mst = Bip44.FromSeed(seed, Bip44Coins.ETHEREUM)
		account = bip44_mst.Purpose().Coin().Account(0).Change(Bip44Changes.CHAIN_EXT).AddressIndex(index)
