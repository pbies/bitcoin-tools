...
def parse_args():
	ap = argparse.ArgumentParser(description='ETH addr generator (batch + multiprocessing, progres + live zapis)')
	...
	with open(args.input, 'rb', buffering=1024*1024) as fh, \
		...
		for results, in_count in pool.imap_unordered(_worker_batch, batch_source(), chunksize=chunksize):
			if results:
				...
				out.flush()  # <<<<<< natychmiastowy zapis
				os.fsync(out.fileno())  # pewność że na dysku (wolniej, ale pewniej)
