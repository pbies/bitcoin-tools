...
def bip39(seed: bytes, x: int, y: int):
	bip32_root_key_obj = bip32utils.BIP32Key.fromEntropy(seed)
	bip32_child_key_obj = bip32_root_key_obj.ChildKey(84 + bip32utils.BIP32_HARDEN).ChildKey(0 + bip32utils.BIP32_HARDEN).ChildKey(0 + bip32utils.BIP32_HARDEN).ChildKey(int(x)).ChildKey(int(y))
...
def process_line(line: str) -> Optional[str]:
	...
	for x in range(4):
		for y in range(1025):
			#try:
			wif=bip39(bytes.fromhex(line), x, y)
			hdwallet.from_wif(wif)
			#except:
			#	continue
...
def read_lines(file_path: Path, chunk_size: int = 10) -> Iterable[list]:
	with file_path.open('r', encoding='utf-8', errors='replace') as f:
		while True:
			chunk = []
...
def main():
	...
	parser.add_argument('-c','--chunk', type=int, default=10, help='number of lines per chunk')
