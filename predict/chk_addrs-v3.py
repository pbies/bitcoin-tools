...
pvk=int(sys.argv[1]) # int pvk

print('Checking for:')
print(f'pvk int = {pvk}')
byt=int_to_bytes3(int(pvk),32)
print(f'pvk hex = {byt.hex()}')
adr=getPublicKey(byt)
print(f'single address uncompressed = {adr}')
key = PrivateKey.from_hex(byt.hex())
a = key.to_public().to_address('P2PKH', compressed=True)
print("single address compressed =",a)
wif=getWif(byt)
print(f'single WIF = {wif}')
...
rng = [0] * (r*2-1)
rng[::2], rng[1::2] = range(pvk, end), range(pvk-1, start, -1)
cnt=len(rng)

for i in tqdm(rng, total=cnt):
	byt=int_to_bytes3(int(i),32)
	key = PrivateKey.from_hex(byt.hex())
	a = key.to_public().to_address('P2PKH', compressed=True)
	if a in content:
		print(f'\n\aFound! int {i} = address {a}')
		print(f'WIF: '+getWif(byt))
		with open('found.txt','a') as f:
			f.write(f'Found! int {i} = address {a}\n')
