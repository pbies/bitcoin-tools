...
def ensure_wallet_exists(name:str, address:str, viewkey:str|None, spendkey:str|None,
                         restore_height:int|None, cli_bin:str, wallet_dir:str):
	wallet_path = Path(wallet_dir) / name
	if wallet_path.exists() or wallet_path.with_suffix(".keys").exists():
		return

	is_standard = (len(address) == 95 and address.startswith("4"))

	# 1) jeśli mamy spend_key i on naprawdę daje ten adres -> zrób z spend key
	if spendkey:
		derived = _addr_from_spend(cli_bin, spendkey)
		if derived == address and is_standard:
			args = [cli_bin, "--generate-from-spend-key", str(wallet_path),
			        "--password","", "--log-file","/dev/null"]
			if restore_height is not None:
				args += ["--restore-height", str(restore_height)]
			rc, out = _run(args, stdin=spendkey.strip()+"\n")
			if rc != 0:
				raise RuntimeError(f"[{name}] create(full-from-spend) failed: {out}")
			return

	# 2) fallback: watch-only z view key (saldo bez wydawania)
	if viewkey and is_standard:
		args = [cli_bin, "--generate-from-view-key", str(wallet_path),
		        "--password","", "--log-file","/dev/null"]
		if restore_height is not None:
			args += ["--restore-height", str(restore_height)]
		stdin_data = f"{address.strip()}\n{viewkey.strip()}\n"
		rc, out = _run(args, stdin=stdin_data)
		if rc != 0:
			raise RuntimeError(f"[{name}] create(watch-only) failed: {out}")
		return

	# 3) jeśli tu trafisz -> brak poprawnej pary lub brak view key
	raise RuntimeError("Address↔keys mismatch: spend key doesn't yield this address and no valid view key to fall back.")

def run_cli(args:list, stdin_data:str=""):
	"""
	Uruchamia monero-wallet-cli z podanymi argumentami.
	Zwraca (rc, stdout).
	"""
	cp = subprocess.run(
		[CLI_BIN] + args,
...
def refresh_and_get_balance(name:str):
	"""
	Uruchamia monero-wallet-cli na istniejącym portfelu:
	- wymusza English
	- refresh
	- balance
	Zwraca (balance, unlocked) jako stringi XMR.
	"""
	wallet_path = Path(WALLET_DIR) / name
	...
	rc, out = run_cli(args)
	if rc != 0:
		raise RuntimeError(f"[{name}] balance failed (rc={rc}). Output:\n{out}")
...
def process_row(idx:int, row:dict):
	...
	try:
		...
		bal, unlk = refresh_and_get_balance(name)
