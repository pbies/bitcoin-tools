...
def main():
	...
	tmp=open(args.output, "w")
	tmp.write('address,view_key,spend_key\n')
	tmp.close()
	...
	for line in tqdm(lines):
		...
		if "," in line:
			# tryb WERYFIKACJI: "expected_addr,hex"
			exp_addr, h = [x.strip() for x in line.split(",", 1)]
			try:
				info = derive_address(h, args.input_endian, args.prefix)
				addr = info["address"]
				ok_fmt = "[OK]" if is_valid_mainnet_address(addr) else "[BAD]"
				match = (addr == exp_addr)
			...
			with open(args.output, "a") as o:
				o.write(f"Given:    {exp_addr}\n")
				o.write(f"Computed: {addr}  {ok_fmt}\n")
				o.write(f"Match:    {'YES' if match else 'NO'}\n")
			...
			with open(args.output, "a") as o:
				o.write(f"{info['address']},{info['private_view_hex']},{info['private_spend_hex']}\n")
