...
def refresh_and_get_balance(name, cli_bin, wallet_dir,
                            daemon_addr, do_refresh=True):
	from pathlib import Path
	wallet_path = Path(wallet_dir) / name
	args = [
	...
	if do_refresh:
		args += ["--daemon-address", daemon_addr,
		         "--trusted-daemon", "--command", "refresh"]
	# balans + wyjście, żeby się nie zawieszał
