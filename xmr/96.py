...
def ensure_wallet_exists(name:str, address:str, viewkey:str, spendkey:str, restore_height:int|None):
	"""
	Jeśli pliki portfela nie istnieją – tworzy je przez monero-wallet-cli --generate-from-keys
	Obsługuje zarówno pełny portfel (z spend key), jak i watch-only (bez spend key).
	"""
	...
	args = [
		"--generate-from-keys", str(wallet_path),
		"--password", WAL_PASS,
	...
	# monero-wallet-cli w tym trybie wymaga w stdin kolejno:
	# Standard address, View key (private), Spend key (private albo Enter dla watch-only)
	stdin_parts = [address.strip(), viewkey.strip()]
	if spendkey:
		stdin_parts.append(spendkey.strip())
	else:
		stdin_parts.append("")  # Enter => watch-only
	stdin_data = "\n".join(stdin_parts) + "\n"

	rc, out = run_cli(args, stdin_data=stdin_data)
	if rc != 0:
		raise RuntimeError(f"[{name}] generate-from-keys failed (rc={rc}). Output:\n{out}")
...
def process_row(idx:int, row:dict):
	...
	name = f"xmr_{idx:06d}"
	try:
		ensure_wallet_exists(name, address, viewkey, spendkey, restore_height)
