...
def main():
	open('output.txt','w')
	for private_spend_key_hex in open('input.txt','r').read().splitlines():
		try:
			info = derive_address_from_spend(private_spend_key_hex, 0x12)
		except:
			continue
		with open('output.txt','a') as o:
			o.write("Address: ")
			o.write(info["address"])
			o.write(f"\n  Public spend: {info["public_spend_hex"]}\n")
