...
def count_csv_rows(path):
	"""Count data rows (non-blank, non-comment, non-header) in a CSV."""
	count = 0
	header_seen = False
	with open(path) as f:
		for line in f:
...
def iter_csv(path):
	"""
	Streaming CSV parser — yields one dict at a time, never loads full file.
	...
	with open(path, newline="") as f:
		...
		for raw in f:
			...
			try:
				...
				yield {"r": r, "s": s, "z": z, "label": label}
				i += 1
			except Exception as e:
				print(f"[!] Skipping row {i}: {e}", file=__import__("sys").stderr)
				i += 1
...
def attack_known_k(csv_path, nonces, out_path):
	...
	print(f"\n[*] Streaming {csv_path}  ({total} rows)...")
	with tqdm(total=total, unit='row', desc='known-k') as bar:
...
def attack_nonce_reuse(csv_path, out_path):
	...
	from collections import defaultdict
	total_p1 = count_csv_rows(csv_path)
	r_count = defaultdict(int)
	with tqdm(total=total_p1, unit='row', desc='pass-1') as bar:
		for row in iter_csv(csv_path):
			r_count[row['r']] += 1
			bar.update(1)
	...
	total_p2 = count_csv_rows(csv_path)
	with tqdm(total=total_p2, unit='row', desc='pass-2') as bar:
		for row in iter_csv(csv_path):
			if row['r'] in dup_r:
...
def attack_pubkey_recovery(csv_path, out_path):
	...
	scanned = 0
	total = count_csv_rows(csv_path)
	with tqdm(total=total, unit='row', desc='recover') as bar:
		for row in iter_csv(csv_path):
			r, s, z, label = row['r'], row['s'], row['z'], row['label']
