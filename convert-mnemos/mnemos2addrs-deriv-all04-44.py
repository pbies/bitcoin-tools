	...
	for a in range(0,3):
		for b in range(0,3):
			#for c in range(0,3):
				#for i in range(0, 3):
			try:
				hdwallet = HDWallet(cryptocurrency=BTC, hd=BIP44HD)
				hdwallet.from_mnemonic(mnemonic=BIP39Mnemonic(mnemonic=mnemo))
				derivation_path = f"m/44'/{str(a)}'/{str(b)}'/{str(c)}/0"
				bip44_derivation: BIP44Derivation = BIP44Derivation()
				bip44_derivation.from_coin_type(coin_type=a)
				bip44_derivation.from_account(account=b)
				bip44_derivation.from_change(change="external-chain")
				bip44_derivation.from_address(address=0)
				#print(bip44_derivation.path())
				o.write(f'{mnemo} : {derivation_path} : wif=')
				hdwallet.from_derivation(bip44_derivation)
