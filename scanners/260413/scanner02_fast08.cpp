/*
 * Bitcoin P2PKH address scanner — multithreaded C++17.
 ...
 *   Reads P2PKH addresses from addrs.txt (one per line).
 *   Appends hits to found.txt:
 *     priv_int=<dec>|wif=<wif>|pubkey=<hex>|address=<b58check>
 */
...
static int8_t B58D[256];
static void b58_init(){std::memset(B58D,-1,sizeof(B58D));for(int i=0;i<58;++i)B58D[(uint8_t)B58A[i]]=(int8_t)i;}
...
static bool addr_to_h160(const std::string& addr,uint8_t h160[20]){
	std::vector<uint8_t> dg; dg.reserve(addr.size());
	for(char c:addr){int v=B58D[(uint8_t)c];if(v<0)return false;int carry=v;for(auto&d:dg){carry+=(int)d*58;d=carry&0xFF;carry>>=8;}while(carry){dg.push_back(carry&0xFF);carry>>=8;}}
	int lead=0; for(char c:addr){if(c=='1')++lead;else break;}
	uint8_t raw[25]={}; int di=(int)dg.size()-1;
	for(int i=lead;i<25&&di>=0;++i,--di)raw[i]=dg[di];
	uint8_t ck[32]; sha256d(raw,21,ck);
	if(std::memcmp(raw+21,ck,4)!=0||raw[0]!=0x00)return false;
	std::memcpy(h160,raw+1,20); return true;
}
static std::string h160_to_addr(const uint8_t h[20]){
	uint8_t p[25]; p[0]=0; std::memcpy(p+1,h,20);
	uint8_t ck[32]; sha256d(p,21,ck); std::memcpy(p+21,ck,4);
	return b58enc(p,25);
}
	...
	while(!g_found.load(std::memory_order_relaxed)){
		...
		for(uint64_t i=0;i<BATCH_SIZE;++i){
			...
			if(targets->count(key)){
				...
				std::string addr=h160_to_addr(h160_buf);
				...
				std::cout<<"\n\aFound! Address: "<<addr<<std::endl;
				std::lock_guard<std::mutex> lk(g_out_mutex);
				g_found_file<<"priv_int="<<priv_dec<<"|wif="<<w<<"|pubkey="<<pk_hex.str()<<"|address="<<addr<<"\n";
				g_found_file.flush();
				goto done;
			}
		}
...
int main(int argc,char* argv[]){
	...
	std::ifstream af("addrs.txt");
	if(!af){std::cerr<<"Error: addrs.txt not found!\n";return 1;}
	...
	while(std::getline(af,line)){
		...
		if(addr_to_h160(line,h.data())) targets.insert(h);
