...
def go(k):
	...
	try:
		...
		tmp7=None
	try:
		hdwallet=HDWallet(cryptocurrency=BTC, hd=BIP84HD).from_private_key(private_key=hex(int(k))[2:])
		tmp8=hdwallet
	except:
		tmp8=None
	...
	for i in [tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7]:
		if i!=None:
			pvk=int(i.private_key(), 16)
			for j in range (-10, 11):
				x=hex(pvk+j)[2:].zfill(64)
				try:
					wif=pvk_to_wif2(x)
					wal=HDWallet(cryptocurrency=BTC, hd=BIP84HD).from_private_key(private_key=x)
				except:
					continue
				r=f'{k}\n{wif}\n{wal.wif()}\n{wal.address("P2PKH")}\n{wal.address("P2SH")}\n{wal.address("P2TR")}\n{wal.address("P2WPKH")}\n{wal.address("P2WPKH-In-P2SH")}\n{wal.address("P2WSH")}\n{wal.address("P2WSH-In-P2SH")}\n\n'
				outfile.write(r)
				outfile.flush()
