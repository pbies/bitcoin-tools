...
set BITCOIN_CLI=D:\111\bitcoin-cli.exe
...
for %%W in ("%WALLETS_DIR%\*.dat") do (
    ...
    for /f "usebackq delims=" %%P in ("%PASSWORDS_FILE%") do (
        if !FOUND!==0 (
            %BITCOIN_CLI% -rpcwallet="!WALLET_FILE!" walletpassphrase "%%P" 5 >nul 2>&1
            if !errorlevel!==0 (
                echo FOUND - Wallet: !WALLET_NAME! Password: %%P
                >> "%RESULTS_FILE%" echo FOUND - Wallet: !WALLET_NAME! Password: %%P
                %BITCOIN_CLI% -rpcwallet="!WALLET_FILE!" walletlock >nul 2>&1
                set FOUND=1
            )
        )
    )
