/*
 * bws2wifs-addrs-seed.cpp
 *
 * Input:  input-seed.txt — one line per entry, 128 hex chars (64 bytes) of
 *         BIP39 seed (the output of PBKDF2, already computed).
 *
 * Pipeline per line:
 *   seed (64 bytes)
 ...
 *
 * Build:
 *   g++ -O3 -std=c++17 -pthread bws2wifs-addrs-seed.cpp -lsecp256k1 -lssl -lcrypto -o bws2wifs-addrs-seed
 *
 * Usage:
 *   ./bws2wifs-addrs-seed [input-seed.txt] [output.txt]
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// Parse input line: exactly 128 hex chars (64 bytes seed)
// First 128 hex chars taken if line is longer.
// ─────────────────────────────────────────────────────────────────────────────
static bool parse_seed(const std::string& line, uint8_t seed[64])
{
    std::string s = line;
    while (!s.empty() && (s.back()=='\r'||s.back()=='\n'||s.back()==' ')) s.pop_back();
    for (char c : s) if (hex_nibble(c) < 0) return false;
    //if (s.size() < 128) return false;
    if (s.size() > 128) s = s.substr(0, 128);
    std::string x = zfill(s, 128);
    return hex_to_bytes(x, seed, 64);
}
                                  ...
                                  secp256k1_context* ctx)
{
    ...
    for (const auto& line : lines) {
        uint8_t seed[64];
        if (!parse_seed(line, seed)) continue;
...
int main(int argc, char* argv[])
{
    ...
    const char* out_path = (argc>2) ? argv[2] : "output-s.txt";
