/*
 * bws2wifs-addrs-entropy.cpp
 *
 * Input:  input-entropy.txt — one line per entry, 64 hex chars (32 bytes) of
 *         BIP39 entropy (256-bit / 24-word mnemonic), no passphrase.
 *
 * Pipeline per line:
 *   entropy (32 bytes)
 *     → BIP39 mnemonic (24 words)
 *     → BIP39 seed via PBKDF2-HMAC-SHA512(pass=mnemonic, salt="mnemonic", 2048 iters, 64 bytes)
 *     → BIP32 master key via HMAC-SHA512("Bitcoin seed", seed)
 *     → child key at m/44'/0'/0'/0/0
 *     → WIF (uncompressed + compressed) + all address types
 *
 * Build:
 *   g++ -O3 -std=c++17 -pthread bws2wifs-addrs-entropy.cpp -lsecp256k1 -lssl -lcrypto -o bws2wifs-addrs-entropy
 *
 * Usage:
 *   ./bws2wifs-addrs-entropy [input-entropy.txt] [output.txt]
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// Parse input line: exactly 64 hex chars (first 64 taken if longer)
// ─────────────────────────────────────────────────────────────────────────────
static bool parse_entropy(const std::string& line, uint8_t entropy[32])
{
    std::string s = line;
    ...
    std::string x = zfill(s, 64);
    return hex_to_bytes(x, entropy, 32);
}

// ─────────────────────────────────────────────────────────────────────────────
// Process one block of lines
// ─────────────────────────────────────────────────────────────────────────────
static std::string process_block(const std::vector<std::string>& lines,
                                  secp256k1_context* ctx)
{
    std::string out;
    out.reserve(lines.size() * 600);

    for (const auto& line : lines) {
        uint8_t entropy[32];
        if (!parse_entropy(line, entropy)) continue;
        ...
        uint8_t key32[32];
        if (!derive_m44(master, ctx, key32)) continue;
...
int main(int argc, char* argv[])
{
    ...
    const char* out_path = (argc>2) ? argv[2] : "output-e.txt";
    ...
    unsigned int nworkers = std::max(1u, 16u);
