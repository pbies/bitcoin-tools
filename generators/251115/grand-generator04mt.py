...
def process_combination(args):
	"""
	Process a single combination of i, j, k, l
	"""
	i, j, k, l = args
	p = i + k
	q = j + l
	...
	# Prepare output
	output = f"pvk hex: {private_key_int:064x}\n"
	output += f"WIF Unc: {wif_uncompressed}\n"
	output += f"WIF Comp: {wif_compressed}\n"
	output += f"Addr Unc: {address_uncompressed}\n"
	output += f"Addr Comp: {address_compressed}\n"
...
def main():
	...
	# Process combinations using ThreadPoolExecutor
	with Pool(processes=24) as pool:
		for result in pool.imap_unordered(process_combination, combinations(), chunksize=10):
			t.update(1)
